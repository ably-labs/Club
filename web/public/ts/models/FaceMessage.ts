...
export default class FaceMessage {
    ...
    static fromFlatBuffer(array: Uint8Array): FaceMessage {
      ...
      // need to create a new int8Array first to avoid using the original buffer
      // which will not be directly useful as a DataView. This issue also happened with google-protobuf
      ...
      return new FaceMessage(coordinates, faceMeshColor, userState.faceMeshSize());
    }
    ...
    encodeIntoFlatBuffer(): Uint8Array {
      ...
      const faceMeshIntArray = new Int8Array(new DataView(this.coordinates.buffer).buffer);
      const faceMesh = UserState.createFaceMeshVector(builder, faceMeshIntArray);
